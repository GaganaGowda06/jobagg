"""Fetch Workable jobs, store them in jobagg.db, and classify the unclassified
...
from jobagg.sources.workable import fetch_batch

# Workable accounts confirmed live by probe. Grow this list via
# scripts/probe_workable_slugs.py before adding names here.
WORKABLE_SLUGS = [
    "g-mass",
]
...
def main() -> None:
    conn = connect()
    print(f"Fetching from {len(WORKABLE_SLUGS)} companies...")
    jobs = fetch_batch(WORKABLE_SLUGS)
